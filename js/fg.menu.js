...
//		 console.log('before create');
		...
		container.parent().show().on("click",function(){ menu.kill(); return false; });
		container.hide().slideDown(options.showSpeed).find('.fg-menu:eq(0)');
		menu.menuOpen = true;
		caller.removeClass(options.loadingState);
		$(document).on("click",killAllMenus);
...
Menu.prototype.flyout = function(container, options) {
	...
	container.addClass('fg-menu-flyout').find('li:has(ul)').each(function(){
		...
		$(this).find('a:eq(0)').addClass('fg-menu-indicator').html('<span>' + $(this).find('a:eq(0)').text() + '</span><span class="ui-icon '+options.nextMenuLink+'"></span>').on({
			mouseenter:function(){
